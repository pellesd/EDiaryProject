...
class SendAdmonitoryFragment : Fragment(R.layout.fragment_send_judgement) {

    private lateinit var binding: FragmentSendJudgementBinding
    private lateinit var viewModel:  TeacherViewModel

    // admonitoryTypes
    private lateinit var admonitoryTypes: List<IntStringResponse>
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        viewModel.admonitoryTypes.observe(viewLifecycleOwner, Observer {
            ...
            when(it) {
                ...
                is Resource.Failure -> {
                    handleApiError(it) { viewModel.admonitoryTypes() }
                }
                is Resource.Loading -> {}
            }
        })
        ...
        viewModel.admonitoryResponse.observe(viewLifecycleOwner, Observer {
            binding.loading.visible(it is Resource.Loading)
            when(it) {
                is Resource.Success -> {
        ...
        binding.submit.setOnClickListener {
            ...
            viewModel.admonitoryToStudent(NewJudgementRequest(
    ...
    private fun fillTypesSpinner(types: List<IntStringResponse>) {
        admonitoryTypes = types

        val options = ArrayList<String>()
        for (admonitoryType in admonitoryTypes)
            options.add(admonitoryType.string.trim())
        ...
        typesSpinner?.onItemSelectedListener = object : AdapterView.OnItemSelectedListener{
            ...
            override fun onItemSelected(parent: AdapterView<*>?, view: View?, position: Int, id: Long) {
                ...
                typeId = getIntByList(admonitoryTypes, item)
            }
        }
    }
